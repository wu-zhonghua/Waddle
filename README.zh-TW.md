...
Waddle 是一款開源、整合 AI 的終端機應用程式，支援 macOS、Linux 與 Windows。它可以搭配任何 AI 模型使用——自行提供 OpenAI、Claude 或 Gemini 的 API 金鑰，或透過 Ollama 與 LM Studio 執行本地模型，完全不需要註冊帳號。

Waddle 同時支援**持久化 SSH 連線**，即使網路中斷或應用程式重新啟動，連線也會自動恢復。你可以使用內建的圖形化編輯器直接編輯遠端檔案，也能在不離開終端機的情況下即時預覽檔案內容。
...
Waddle AI 不只是一個聊天機器人——它能直接讀取你的終端機輸出、分析目前開啟的小工具（Widget），還能執行檔案操作。當你在 Debug 時，AI 能看到你的錯誤訊息並給予針對性的建議，而不是泛泛的回答。
...
詳細說明請參閱 [Waddle AI 文件](https://docs.waddle.dev/waveai) 與 [Waddle AI Modes 文件](https://docs.waddle.dev/waveai-modes)。

### 🔗 持久化 SSH 連線

傳統的 SSH 連線在網路不穩時就會斷開，你得重新連線、重新切換目錄、重新啟動程式。Waddle 的持久化 SSH 連線徹底解決了這個痛點——連線中斷後會自動重新建立，你的工作階段（Session）完整保留，就像什麼都沒發生過一樣。

- 連線中斷、網路切換、Waddle 重啟後自動重新連線
- 工作階段狀態完整保留
- 一鍵即可連線遠端伺服器，完整存取終端機與檔案系統

### 🧩 彈性拖放介面

Waddle 的介面由可自由排列的「區塊（Block）」組成。你可以將終端機、編輯器、網頁瀏覽器、AI 助手像拼圖一樣排列在同一個畫面中，打造最適合你工作流程的佈局。每個區塊都能一鍵切換全螢幕，放大查看後立即回到多區塊視圖。

### ✏️ 內建編輯器

不需要額外開啟 VS Code 或 Vim——Waddle 內建的圖形化編輯器支援語法高亮與現代編輯功能，可以直接編輯本地或遠端檔案。對於需要快速修改設定檔或程式碼的場景特別方便。
...
`wsh` 是 Waddle 提供的強大 CLI 工具，讓你從命令列管理整個工作空間：

- 在不同終端機連線間共享資料
- 透過 `wsh file` 在本地與遠端 SSH 主機之間無縫複製和同步檔案
- 從命令列直接控制 Waddle 的介面佈局

## 安裝

Waddle 支援 macOS、Linux 與 Windows。

各平台的安裝說明請參閱[此處](https://docs.waddle.dev/gettingstarted)。

你也可以直接從官方下載頁面安裝：[www.waddle.dev/download](https://www.waddle.dev/download)。
...
Waddle 持續進化中！發展藍圖會隨每次發行版本持續更新，請至[此處](./ROADMAP.md)查閱。

想為未來版本提供建議？歡迎加入 [Discord](https://discord.gg/XfvZ334gwU) 社群，或提交 [Feature Request](https://github.com/waddledev/waddle/issues/new/choose)！

## 連結

- 官方網站 &mdash; https://www.waddle.dev
- 下載頁面 &mdash; https://www.waddle.dev/download
- 技術文件 &mdash; https://docs.waddle.dev
- X（Twitter）&mdash; https://x.com/waddledev
- Discord 社群 &mdash; https://discord.gg/XfvZ334gwU

## 從原始碼建置

請參閱 [Building Waddle](BUILD.md)。

## 貢獻

Waddle 使用 GitHub Issues 進行問題追蹤。
...
Waddle 採用 Apache-2.0 授權條款。相依性資訊請參閱[此處](./ACKNOWLEDGEMENTS.md)。
