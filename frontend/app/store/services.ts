...
function callBackendService(waveEnv: WaddleEnv, service: string, method: string, args: any[], noUIContext?: boolean): Promise<any> {
    if (waveEnv != null) {
...
export class WindowServiceType {
    ...
    CreateWindow(winSize: WinSize, workspaceId: string): Promise<WaddleWindow> {
        return callBackendService(this?.waveEnv, "window", "CreateWindow", Array.from(arguments))
    }
    GetWindow(windowId: string): Promise<WaddleWindow> {
