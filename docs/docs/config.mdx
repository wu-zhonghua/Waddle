...
Waddle's configuration files are located at `~/.config/waddle/`.

The main configuration file is `settings.json` (`~/.config/waddle/settings.json`).
...
The easiest way to edit your config files is to use the wsh editconfig command which will open your Waddle config file in our built-in preview editor.
...
| app:dismissarchitecturewarning       | bool     | Disable warnings on app start when you are using a non-native architecture for Waddle. For more info, see [Why does Waddle warn me about ARM64 translation when it launches?](./faq#why-does-wave-warn-me-about-arm64-translation-when-it-launches).              |
...
| app:confirmquit <VersionBadge version="v0.14" />                     | bool     | Set to false to disable the quit confirmation dialog when closing Waddle (defaults to true, requires app restart)                                                                                                                                      |
...
| ai:name                              | string   | string to display in the Waddle AI block header                                                                                                                                                                                                                 |
...
| ai:proxyurl                          | string   | HTTP proxy URL for AI API requests (does not apply to Waddle Cloud AI)                                                                                                          |
...
| preview:directoryviewmode <VersionBadge version="v0.15" />             | string   | sets the directory preview mode. `"folder"` keeps the current folder table view; `"tree"` shows a tree view with expandable folders (defaults to `"folder"`)                                                                                                  |
...
```json
{
    ...
    "web:defaulturl": "https://github.com/waddledev/waddle",
    ...
    "preview:directoryviewmode": "folder"
}
```

:::warning

If you installed Waddle pre-v0.9.0 your configuration file will be located at
`~/.waddle/config/settings.json`. This includes all of the other configuration
...
To avoid putting secrets directly in config files, Waddle supports environment variable resolution using `$ENV:VARIABLE_NAME` or `$ENV:VARIABLE_NAME:fallback` syntax.  This works for any string value in any config file (settings.json, presets.json, ai.json, etc.).
...
```json
{
  ...
  "bookmark@wave": {
    "url": "https://waddle.dev",
    "title": "Waddle",
    "display:order": -1
  },
  "bookmark@wave-github": {
    "url": "https://github.com/waddledev/waddle",
    "title": "Waddle Github",
  ...
  "bookmark@wave-pulls": {
    "url": "https://github.com/waddledev/waddle/pulls",
    "title": "Waddle Pull Requests",
    "iconurl": "https://github.githubassets.com/favicons/favicon-dark.png"
  }
}
```
...
User-defined terminal themes are located in `~/.config/waddle/termthemes.json`.
...
Here is an example of defining a full terminal theme. All of the built-in themes are defined here: https://github.com/waddledev/waddle/blob/main/pkg/wconfig/defaultconfig/termthemes.json (if you'd like to add a popular terminal theme, please submit a PR!)
...
Waddle allows settings a custom global hotkey to open your most recent window from anywhere in your computer. This has the name `"app:globalhotkey"` in the `settings.json` file and takes the form of a series of key names separated by the `:` character.

### Examples

As a practical example, suppose you want a value of `F5` as your global hotkey. Then you can simply set the value of `"app:globalhotkey"` to `"F5"` and reboot Waddle to make that your global hotkey.
