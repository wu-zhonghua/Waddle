...
You can use \`wsh view [path]\` from any Waddle terminal window to open a preview widget with the contents of the specified path (e.g. `wsh view .` or `wsh view ~/myimage.jpg`).
...
In the directory preview, select a local file and press <Kbd k="Space" /> to open it with the operating system's native preview/open behavior. On macOS this uses Quick Look. On Linux and Windows it opens the file with the system default application.
