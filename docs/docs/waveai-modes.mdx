...
id: "waveai-modes"
title: "Waddle AI (Local Models + BYOK)"
---
...
Waddle AI supports custom AI modes that allow you to use local models, custom API endpoints, and alternative AI providers. This gives you complete control over which models and providers you use with Waddle's AI features.

## Configuration Overview

AI modes are configured in `~/.config/waddle/waveai.json`.

**To edit using the UI:**
1. Click the settings (gear) icon in the widget bar
2. Select "Settings" from the menu
3. Choose "Waddle AI Modes" from the settings sidebar
...
Waddle AI now supports provider-based configuration which automatically applies sensible defaults for common providers. By specifying the `ai:provider` field, you can significantly simplify your configuration as the system will automatically set up endpoints, API types, and secret names.
...
Waddle AI supports the following API types:
...
## Global Waddle AI Settings

You can configure global Waddle AI behavior in your Waddle settings (separate from the mode configurations in `waveai.json`).

### Setting a Default AI Mode

After configuring a local model or custom mode, you can make it the default by setting `waveai:defaultmode` in your Waddle settings.
...
This will make the specified mode the default selection when opening Waddle AI features.

:::note
Waddle AI normally requires telemetry to be enabled. However, if you configure your own custom model (local or BYOK) and set `waveai:defaultmode` to that custom mode's key, you will not receive telemetry requirement messages. This allows you to use Waddle AI features completely privately with your own models. <VersionBadge version="v0.13.1"/>
:::

### Hiding Waddle Cloud Modes

If you prefer to use only your local or custom models and want to hide Waddle's cloud AI modes from the mode dropdown, set `waveai:showcloudmodes` to `false`:
...
This will hide Waddle's built-in cloud AI modes, showing only your custom configured modes.
...
The `ai:apitokensecretname` should be the name of an environment variable that contains your API key. Set this environment variable before running Waddle.
...
Instead of storing API keys directly in the configuration, you should use Waddle's secret store to keep your credentials secure. Secrets are stored encrypted using your system's native keychain.
...
The `openai` provider automatically looks for the `OPENAI_KEY` secret. See the [Secrets documentation](./secrets.mdx) for more information on managing secrets securely in Waddle.
...
If Waddle can't connect to your model server:
...
| `waveai:cloud` | No | Internal - for Waddle Cloud AI configuration only |
| `waveai:premium` | No | Internal - for Waddle Cloud AI configuration only |
...
:::warning
If you don't include `"tools"` in the `ai:capabilities` array, the AI model will not be able to interact with your Waddle terminal widgets, read/write files, or execute commands. Most AI modes should include `"tools"` for the best Waddle experience.
:::
