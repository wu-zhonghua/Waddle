...
Waddle allows users to connect to various machines and unify them together in a way that preserves the unique behavior of each. At the moment, this extends to SSH remote connections and local WSL connections.
...
`wsh` is a small program that helps manage waddle regardless of which machine you are currently connected to. It is always included on your host machine, but you also have the option to install it when connecting to SSH and WSL Connections. If it is installed on the connection, it is installed at `~/.waddle/bin/wsh`. Then, when wave connects to your connection (and only when wave connects to your connection), the following happens:

- `~/.waddle/bin` is added to your `PATH` for that individual session. This allows the user to use the `wsh` command without providing the complete path.
...
If this fails for some reason, Waddle will attempt to run without `wsh`. You will see this indicated by a small **<code><i className="fa-link-slash fa-solid fa-sharp"/></code>** icon in the block header. For more info on what `wsh` is capable of, see [wsh command](/wsh). And if you wish to view the source code of `wsh`, you can find it [here](https://github.com/waddledev/waddle/tree/main/cmd/wsh).
...
| -------------------- | ----------------------------------------------------------------------------- |
| TERM_PROGRAM         | Set to `waddle` in wave.                                                    |
| WADDLE             | This is set to 1 in wave.                                                     |
| WADDLE_BLOCKID     | The id of the block containing your current terminal widget.                  |
| WADDLE_CLIENTID    | The id of the RPC Client being used by your current terminal widget.          |
| WADDLE_CONN        | The name of the remote connection being used by your current terminal widget. |
| WADDLE_TABID       | The id of the tab containing your current terminal widget.                    |
| WADDLE_VERSION     | The current semver version of Waddle.                                         |
| WADDLE_WORKSPACEID | The id of thw workspace containing your current terminal widget.              |

# Initialization Scripts

Waddle provides you with options for running initialization scripts on your remote machines when connecting to them. These are defined in `connections.json` and can take either the form of the path of a script or a short script written directly in the file. If multiple scripts are defined, the most specific one relevant to the current shell is applied. The keywords for the scripts are:
...
| conn:wshpath | A string indicating the path to the `wsh` executable on the connection. It defaults to `"~/.waddle/bin/wsh"`.|
...
Waddle resolves the identity agent path in this order:

- If `ssh:identityagent` (or `IdentityAgent` in SSH config) is set for the connection, that socket or pipe is used.
- If not set on Windows, Waddle falls back to the built-in OpenSSH agent pipe `\\.\pipe\openssh-ssh-agent`. Ensure the **OpenSSH Authentication Agent** service is running.
- If not set on macOS/Linux, Waddle queries your shell environment for `SSH_AUTH_SOCK` to detect the agent path automatically.
...
While Waddle provides an option disable `wsh` when first connecting to a remote, there are cases where you may wish to disable it afterward. The easiest way to do this is by editing the `connections.json` file. Suppose the connection shows up in the dropdown as `root@wshless`. Then you can disable it manually with the following line:
...
Waddle is capable of injecting `wsh` in the following shells:
