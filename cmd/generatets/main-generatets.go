...
import (
	...
	"github.com/waddledev/waddle/pkg/tsgen"
...
func generateTypesFile(tsTypesMap map[reflect.Type]string) error {
	...
	tsgen.GenerateWaddleObjTypes(tsTypesMap)
	tsgen.GenerateWaddleEventTypes(tsTypesMap)
...
func generateWaddleEventFile(tsTypesMap map[reflect.Type]string) error {
	...
	fmt.Fprintf(&buf, "// generated by cmd/generate/main-generatets.go\n\n")
	fmt.Fprintf(&buf, "declare global {\n\n")
	fmt.Fprint(&buf, utilfn.IndentString("    ", tsgen.GenerateWaddleEventTypes(tsTypesMap)))
	fmt.Fprintf(&buf, "}\n\n")
	fmt.Fprintf(&buf, "export {}\n")
	written, err := utilfn.WriteFileIfDifferent(fileName, buf.Bytes())
	if !written {
...
func generateServicesFile(tsTypesMap map[reflect.Type]string) error {
	...
	fmt.Fprintf(&buf, "import type { WaddleEnv } from \"@/app/waveenv/waveenv\";\n\n")
	fmt.Fprintf(&buf, "function callBackendService(waveEnv: WaddleEnv, service: string, method: string, args: any[], noUIContext?: boolean): Promise<any> {\n")
...
func main() {
	...
	err = generateWaddleEventFile(tsTypesMap)
	if err != nil {
