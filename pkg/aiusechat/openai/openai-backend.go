...
) (*uctypes.WaddleStopReason, []*OpenAIChatMessage, *uctypes.RateLimitInfo, error) {
	if sse == nil {
...
func handleOpenAIStreamingResp(ctx context.Context, sse *sse.SSEHandlerCh, decoder *eventsource.Decoder, cont *uctypes.WaddleContinueResponse, chatOpts uctypes.WaddleChatOpts) (*uctypes.WaddleStopReason, []*OpenAIChatMessage) {
	// Per-response state
...
func handleOpenAIEvent(
	...
	state *openaiStreamingState,
	cont *uctypes.WaddleContinueResponse,
) (final *uctypes.WaddleStopReason, messages []*OpenAIChatMessage) {
	...
	case "response.created":
		...
		if err := json.Unmarshal([]byte(data), &ev); err != nil {
			_ = sse.AiMsgError(err.Error())
			return &uctypes.WaddleStopReason{Kind: uctypes.StopKindError, ErrorType: "decode", ErrorText: err.Error()}, nil
		}
...
func extractMessageAndToolsFromResponse(resp openaiResponse, state *openaiStreamingState) ([]*OpenAIChatMessage, []uctypes.WaddleToolCall) {
	var messageContent []OpenAIMessageContent
	var toolCalls []uctypes.WaddleToolCall
	...
	for _, outputItem := range resp.Output {
		...
		case "function_call":
			...
			toolCall := uctypes.WaddleToolCall{
