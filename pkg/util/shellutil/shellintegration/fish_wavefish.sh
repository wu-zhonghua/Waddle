...
# Add Waddle binary directory to PATH
...
wsh token "$WADDLE_SWAPTOKEN" fish 2>/dev/null | source
set -e WADDLE_SWAPTOKEN

# Load Waddle completions
wsh completion fish | source

set -g _WADDLE_SI_FIRSTPROMPT 1

# shell integration
function _waddle_si_blocked
...
function _waddle_si_osc7
    _waddle_si_blocked; and return
    # Use fish-native URL encoding
    set -l encoded_pwd (string escape --style=url -- "$PWD")
    printf '\033]7;file://localhost%s\007' $encoded_pwd
end

function _waddle_si_prompt --on-event fish_prompt
    set -l _waddle_si_status $status
    _waddle_si_blocked; and return
    if test $_WADDLE_SI_FIRSTPROMPT -eq 1
    ...
    set -g _WADDLE_SI_FIRSTPROMPT 0
end

function _waddle_si_preexec --on-event fish_preexec
    _waddle_si_blocked; and return
...
function _waddle_si_chpwd --on-variable PWD
    _waddle_si_osc7
end